...
namespace Leayal.VData
{
    ...
    /// </summary>
    public class VDataProcesser : IDisposable
    {
        /// <summary>
        ...
        private XorStream ZipFileStream;
        // General comment
        ...
        /// <summary>
        /// Extract the zip entry's stream
        /// </summary>
        /// <param name="_zfe">Entry information of file to extract</param>
        /// <returns>A stream of the entry or null if error.</returns>
        /// <remarks>Unique compression methods are Store and Deflate</remarks>
        public Stream ExtractFile(ZipFileEntry _zfe)
        {
            // check signature
            byte[] signature = new byte[4];
            this.ZipFileStream.Seek(_zfe.HeaderOffset, SeekOrigin.Begin);
            ...
            // Buffered copy
            this.ZipFileStream.Seek(_zfe.FileOffset, SeekOrigin.Begin);
            this.ZipFileStream.Seek(_zfe.FileOffset + _zfe.CompressedSize, SeekOrigin.End);
            return inStream;
        }
        /// <summary>
        ...
        public static bool RemoveEntries(ref VDataProcesser _zip, List<ZipFileEntry> _zfes)
        {
            ...
            try
            {
                VDataProcesser tempZip = VDataProcesser.Create(tempZipName, string.Empty);
                ...
                _zip = VDataProcesser.Open(_zip.FileName, _zip.Access);
            }
            catch
            {
                return false;
        ...
        /// <returns>True if success, false if not</returns>
        /// <remarks>This method only works for storage of type FileStream</remarks>
        public static bool CloneWithoutEntries(VDataProcesser _zip, string output, List<ZipFileEntry> _zfes)
        {
            if (!(_zip.ZipFileStream is FileStream))
                throw new InvalidOperationException("RemoveEntries is allowed just over streams of type FileStream");
            ...
            try
            {
                VDataProcesser tempZip = VDataProcesser.Create(output, string.Empty);
        ...
        private void Store(ref ZipFileEntry _zfe, Stream _source)
        {
            ...
            do
            {
                ...
                if (bytesRead > 0)
                {
                    ...
                    {
                        _zfe.Crc32 = VDataProcesser.CrcTable[(_zfe.Crc32 ^ buffer[i]) & 0xFF] ^ (_zfe.Crc32 >> 8);
                    }
                }
