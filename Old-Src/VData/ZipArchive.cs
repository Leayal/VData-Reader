...
namespace Leayal
{
    ...
    public class ZipArchive : IDisposable
    {
        ...
        public static ZipArchive Create(string _filename, string _comment)
        {
            Stream stream = new FileStream(_filename, FileMode.Create, FileAccess.ReadWrite);

            ZipArchive zip = Create(stream, _comment);
        ...
        /// <returns>A valid ZipStorer object</returns>
        public static ZipArchive Create(Stream _stream, string _comment)
        {
        ...
        /// <returns>A valid ZipStorer object</returns>
        public static ZipArchive Open(Stream _stream, FileAccess _access)
        {
        ...
        public static bool RemoveEntries(ref ZipArchive _zip, List<ZipFileEntry> _zfes)
        {
            ...
            try
            {
                ZipArchive tempZip = ZipArchive.Create(tempZipName, string.Empty);
                ...
                _zip = ZipArchive.Open(_zip.FileName, _zip.Access);
            }
            catch
            {
                return false;
        ...
        private void Store(ref ZipFileEntry _zfe, Stream _source)
        {
            ...
            do
            {
                ...
                if (bytesRead > 0)
                {
                    ...
                    {
                        _zfe.Crc32 = ZipArchive.CrcTable[(_zfe.Crc32 ^ buffer[i]) & 0xFF] ^ (_zfe.Crc32 >> 8);
                    }
                }
